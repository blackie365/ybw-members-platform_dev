...
Status: [READS LIVE ON POSTGRES — Phase 3 & 4 DONE. Phase 5 IN PROGRESS: write-store seam built (PG primary + Firestore mirror, env-gated by MAGAZINE_STORE=pg); core write paths + builder/admin-reads converted to read/write stores; story_library + idml_drafts reads overd; Firestore removal + backfill remain]
Created: 2026-08-29 (Weekend quiet-time session)
Updated: 2026-08-30 (Phase 3 & 4 complete; Phase 5 write-store wiring merged + builder pages/admin-read conversion)
...
5. **Phase 5 (IN PROGRESS — user chose "Full PG migration, dual-origin transition"; do NOT touch Issuu flip-book):**
   ...
   c. **DONE in latest session (uncommitted, extend write-store PR #440):** builder-pages reads+writes → read/write stores (`page-actions.ts`; PG page identity now injects `docId=String(id)`); admin-builder reads → PG (`edition-actions.ts` issue reads, `reader-edition-actions.ts` fwd sync, `api/admin/reader-editions/[id]/route.ts` GET list/GET by id/DELETE unlink); story-library + IDML-draft reads → PG (`_helpers.ts` `getIssueStoryLibraryCollectionItems`, `story-library-actions.ts`, `idml-import-actions.ts` `loadIdmlDraft`/`loadLatestIdmlDraft`/publish-issue meta+link); `simple-reader.ts` `getReaderEditionByIssueId`/`getReaderEditionById`/`listReaderEditions` now PG-first composite. Added read-store methods `getStoryLibrary`/`listIdmlDrafts`/`getIdmlDraft` (PG+FS+composite). Fixed leftover type bugs in `edition-actions.ts` + `reader-edition-actions.ts` + `pg-store.test.ts` docId. typecheck/lint clean, 113/113 tests green.
   d. **STILL REMAINS in Phase 5:** backfill `magazine_story_library` + `magazine_idml_drafts` from Firestore → PG (extend `scripts/backfill-firestore-to-pg.ts`) BEFORE dropping FS fallback; remaining simple-reader FS reads (`getReaderEditionIdBySlug`, `syncReaderEditionCoverFromIssue`, `syncReaderEditionsForIssue`, `findBuilderIssueBySlug`, `getReaderEditionFromBuilderIssue`, `hydrateEditionWithLegacyPages`) + `syncReaderEditionToLegacyIssue` in `reader-edition-actions.ts`; then make read/write store selectors PG-only (drop composite FS fallback/mirror + firestore-store/firestore-write-store usage), remove Firestore magazine collections + 1MB/IDML chunking workarounds.
   e. User constraint: no existing magazine writes need preserving (will create a new edition if needed); keep Issuu flip-book editions rendering via `/new-edition`.
